...
- Enigmail (needs version >= 2.07):
  ...
  the passphrase). We set it to `timeout` with 5 minutes of delay:
  : gsettings set org.gnome.crypto.cache gpg-cache-method "timeout"
  : gsettings set org.gnome.crypto.cache gpg-cache-ttl 300


Copy the =.gnupg/= folder in =~/=, and ensure permissions are correct:
...
Bug with Enigmail 1.9.6-1 (doesn't recognize gpg); fixed in
1.9.6-2: https://www.mail-archive.com/debian-bugs-dist@lists.debian.org/msg1471698.html
