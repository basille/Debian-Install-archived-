...
** Miscellaneous
  ...
  : sudo apt install autoconf build-essential cmake cmake-curses-gui cowsay debian-goodies detox disper elinks espeak etcher-electron fortune git gnome-common gparted gtick hibernate libcanberra-gtk3-0:i386 mlocate ntp privoxy subversion transmission tree unetbootin units unrar virtualbox wakeonlan
...
- JabFox: Adjust the path to JabRef launcher, usually
  =/usr/bin/jabref= ('extensions.@jabfox.jabRefPath' preference of
  Firefox); Export format to BibTeX;
- JabRef: Activate "Remote operation" under the Advanced tab (port
  6050).
